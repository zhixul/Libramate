...
<html>
	...
	<body onload="performSearch()">
		...
		<form id="myform" action="SearchUser">
				<%User s1 = (User) session.getAttribute("finalUser"); %>
			...
			<div id="inner">
			<div id="left">
				<center><img style="margin-top:30px;border:1px black solid; height:300px; border-radius:50%;" src='<%=s1.getImageURL() %>' ></center>
				<div style="background-color:rgb(65,128,186); width:80%; margin:10%;" >
					<center><h2 class="white">@<%=s1.getUsername() %></h2></center>
					<div class="tab">
					...
					<div id="London" class="tabcontent">
					  ...
					  <%	 String following[] = s1.getFollowing();%>
					...
					<div id="Paris" class="tabcontent">
					  	<%	 String follower[] = s1.getFollowers();%>
			...
			<div id="right">
				<h1 style="font-weight:bold;"><%=s1.getUsername() %>'s Library</h1>
				<%boolean exist = false;
					String follow ="";
				  for(String ss1 : follower)
				  { 
					  if(ss1.equals(session.getAttribute("username")))
					  {
						exist = true;
					  }
				  }
					...
					%>
				<button id="button1" onclick="return follow();" class="logout"><%=follow %></button>
				<div id="shu" style="background-color:steelblue; position:relative; top:40px;">
					<div class="tab2">
						  
						  <button id="shu2" style="width:200px;float:left;" class="tablinks" onclick="openCity2(event, '456')"><font style="font-size:25px;"class="white">Favorite</font></button>
		...
		document.getElementById("shu2").click();
		</script>
		<script>
		function performSearch()
		{
			$(document).ready(function(){
				...
				<% String read[] = (String[]) session.getAttribute("read");
				 String fav[] = s1.getLibrary().getFavorite();%>
...
<script>
	function follow()
	{
		...
		xhttp.open("GET" , "Follow", false); 
		xhttp.send();
		<%if(follow == "Follow"){%>
			document.getElementById("button1").innerHTML = "Unfollow";
		<%}else{%>
			document.getElementById("button1").innerHTML = "Follow";
		<%}%>
		return false;
	}
</script>
